...
- **[P2] 遅延スクロールに世代（有効性）チェックが無かった（確認・修正済み）**: 上記の
  `setTimeout` が `loadData` と同じ世代カウンタ（`articleLoadGenerationRef`）を見ておらず、
  300ms以内にタブ切替・フィルタ/検索変更・別の同期完了などで新しい `loadData` が走ると、
  ユーザーが既に移った先の文脈を巻き戻して先頭へ強制スクロールする実害のある不具合
  だった。Homeタブはバックグラウンドでもマウントされたままのため、**他タブを見ている間に
  発火することもある**。`setTimeout` をスケジュールする直前に `articleLoadGenerationRef.current`
  を捕捉し、発火時に一致しなければ何もしないよう修正した
