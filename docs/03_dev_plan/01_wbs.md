...
### `allowFontScaling`（Dynamic Type / 文字サイズ設定）がアプリ全体で未対応 → TODO

- **経緯**: Pro画面の価格表示で、iOSのDynamic Type設定により`fontSize`が拡大され、
  固定`lineHeight`と衝突してクリップされる不具合が発生（2026-09-15、`ad75873`で
  該当箇所は修正済み）。原因調査中に、`allowFontScaling`をアプリ全体で一度も
  明示的に扱っていないことが判明した
- **現状**: RNの`Text`は既定で`allowFontScaling: true`のため、すべての画面の
  すべてのテキストが端末の文字サイズ設定に応じて拡大される。固定`lineHeight`を
  持つ箇所（バッジ・カード等、レイアウトが窮屈な場所）は同じ理由で今後も
  クリップしうる
- **今回は個別修正で対応**: `lineHeight: undefined`で該当箇所だけ「拡大後の
  fontSizeに応じた自然な行高」に切り替えて解決した。これは固定`lineHeight`が
  必要な箇所すべてに使える汎用的な直し方
- **保留にした理由**: 全画面を対象にした`allowFontScaling`の方針決定（全面的に
  無効化する／全面的に許容してレイアウトを可変対応させる、のどちらを採るか）は
  規模が大きい。今回のような個別の実害が出た箇所から都度直す方針で当面は様子見
- **次に踏むとしたら**: 固定`lineHeight`を持つ箇所（`grep -rn "lineHeight:" app/
  components/`）を洗い出し、大きめの文字サイズ設定でスクリーンショット比較する
