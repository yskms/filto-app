...
> `app.json` の bump は **iOS 1.5.0 の審査通過後**に行う。`runtimeVersion` が `appVersion`
> ポリシーのため、先に上げてしまうと審査中の 1.5.0 に対して OTA を打てなくなる
> （v1.5.0 では実際に提出直後の OTA 修正が必要になった）。
